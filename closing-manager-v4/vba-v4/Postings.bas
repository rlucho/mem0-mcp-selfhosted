...
With sess
    ...
    File = ""
    'V4-CIO FIX: bounded, responsive wait for the printed file. The old loop
    'rebuilt a Shell.Application on every pass with no pause - a tight spin that
    'froze Excel and ended in error 80010108. In this module that freeze could
    'strand a run with entries already posted but not yet verified.
    File = CM_WaitForPrint(FTemp, fso, "ZGE132 re-check (local currency)")
    ...
    'froze Excel and ended in error 80010108. In this module that freeze could
    'strand a run with entries already posted but not yet verified.
    File = CM_WaitForPrint(FTemp, fso, "ZGE132 re-check (group currency)")
...
With sess
    ...
    Else
        ...
        'froze Excel and ended in error 80010108. In this module that freeze could
        'strand a run with entries already posted but not yet verified.
        File = CM_WaitForPrint(FTemp, fso, "ZGLGWUL")
    ...
    Do Until strim.EOS
        ...
        If Left(Trim(line), 1) = "|" Then
            ...
            If CStr(arr(LBound(arr) + 1)) = "44400200" Then
                'V4-CIO FIX: locale-proof amount, and the trailing minus is now stripped
    ...
    If Left(Trim(line), 1) = "|" And Trim(getLineData(line, FirstColumn, 1)) <> FirstColumn Then
        ...
        Else
            If Right(Trim(getLineData(line, "Amount", 1)), 1) = "-" Then
                CheckPosting = True
            'V4-CIO FIX: locale-proof amount (was Round on a SAP string -> error 13)
...
If Left(DocG, 2) <> "XY" Then
    With sess
        ...
        'froze Excel and ended in error 80010108. In this module that freeze could
        'strand a run with entries already posted but not yet verified.
        File = CM_WaitForPrint(FTemp, fso, "ZGR215 (batch-input documents)")
