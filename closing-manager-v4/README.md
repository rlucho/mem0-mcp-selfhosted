...
| 21 | **`Range(row, col)` → `Cells(row, col)`** — `Closing.bas` bolded the GTB1 discrepancy message with `Sheets("Errors").Range(EmptRow, 1)`. `Worksheet.Range` takes an A1 string or a Range, not two numbers, so this raised **error 1004 every time the branch ran** — and it only runs *after* ZGE132/ZGLGWUL have posted. The branch had never worked. | `Closing` | The GTB1 mismatch it exists to report was never written, the close aborted post-posting, and V4's handler blamed a "renamed or protected sheet". |
| 22 | **`ImportTitle` / `ProperArray` bounded** — two loops in the original `GlobalModule` that could not terminate: `ImportTitle` only exits on finding a line starting with `\|`, and `ProperArray` only advances when it finds one. An empty or pipe-less SAP export spun for ever. `CreateArray` is called on `t001.txt`, `zgxmit.txt`, `t001z.txt`, `skb1.txt`, `closestatus.txt`, `zglrme.txt` and `zge132gwul.txt`, so this was reachable. | `GlobalModule` | A silent, unbreakable hang — now an explained "SAP sent an empty report" stop naming the file. |
| 23 | **`Post_ZGLGWUL` handler leak** — `On Error Resume Next` was armed and only disarmed inside the `Else` branch, so on the other path it stayed armed for the rest of the procedure and swallowed everything after it, including the explained amount error. `AA16` was then written from whatever `Am` happened to hold, and fed straight into the GTB1 balance comparison. | `Postings` | A silently wrong number in a balance check. |
| 24 | **Contradictory number formats stop the run** — if two extracts in one close imply different decimal conventions, `CM_Learn` refuses rather than last-write-wins, so one literal cannot be read two ways in a single run. `UpdateData` gains the same handler as the close (its four `On Error GoTo 0` now restore the outer handler instead of disabling it). | `GlobalModule`, `Admin` | Silent inconsistency between reports in the same pack. |
